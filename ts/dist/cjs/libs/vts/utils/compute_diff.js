...
function compute_diff({ new: new_data, old, prefix = "", trim = true, trim_keep = 3 }) {
  ...
  const dumped_lines = [];
  const plus_str = import__.Color.green_bold("+");
  const minus_str = import__.Color.red_bold("-");
  let line_nr = 0;
    ...
    diff_lines[index].walk((line, i, arr) => {
      ...
      if (trim && !part.added && !part.removed && !(i < trim_keep || i >= arr.length - trim_keep)) {
        if (!last_dots && (i === trim_keep || i === arr.length - trim_keep - 1)) {
          dumped_lines.push(`${whitespace_prefix} ${String().padEnd(max_line_nr_length, " ")} | ${line_prefix} ${import__.Color.italic("... unchanged ...")}`);
          last_dots = true;
        }
        return;
      }
