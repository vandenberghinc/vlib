...
async function cjs({ src, dest, target, platform, override = false, debug = 0 }) {
  ...
  for (const esm of esm_files) {
    ...
    skipping...`);
      ...
      if (import__.debug.on(1))
        console.log(`.${esm}
    copied asset to ${out_path} [${import__.Utils.format_bytes(abs_esm.size)}]`);
    ...
    try {
      ...
      if (import__.debug.on(1))
        console.log(`.${esm}
    transformed to ${out_path} [${import__.Utils.format_bytes(Buffer.byteLength(result.code, "utf8"))}]`);
    } catch (err) {
