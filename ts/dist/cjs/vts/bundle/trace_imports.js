...
class ImportTracer {
  ...
  trace(traces, import_limit = 100) {
    ...
    for (const req of traces) {
      ...
      const to_abs = this.resolve_import_specifier(from_abs, req.to) ?? this.normalize_to_abs(req.to);
...
${indent}    ${import_vlib.Color.italic(common_base_slice ? "." + path.resolve(l).slice(common_base_slice) : l)}`).join(import_vlib.Color.blue(" => ")));
  ...
  find_all_paths(start_abs, goal_abs, max_depth) {
    const chains = [];
    const stack = [];
    const expanded_in_trace = /* @__PURE__ */ new Set();
    ...
    dfs(start_abs, 0, /* @__PURE__ */ new Set());
    return chains;
  }
  /**
  ...
  resolve_package_entry(pkg_root, subpath) {
    ...
    if (pkg?.exports !== void 0) {
      ...
      const target = this.resolve_exports_target(exp, key) ?? this.resolve_exports_target(exp, ".");
      if (target) {
