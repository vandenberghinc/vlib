...
cli.command({
  ...
  async callback({ src = [process.cwd()], templates, allow_not_found = false, debug = 0, yes = false }) {
    const { error } = await new import_transformer.Transformer({
      include: src,
...
cli.command({
  ...
  async callback({ src, pkg = "./package.json", allow_not_found = false, debug = 0, yes = false }) {
    const { error } = await new import_transformer.Transformer({
      include: src,
...
cli.command({
  ...
  args: [
    ...
    { id: ["--platform", "-p"], enum: ["browser", "node", void 0], required: false, description: "The ES platform, 'browser' or 'node'." },
  ...
  async callback({ src, dest, target, platform, override = false, debug = 0 }) {
    await (0, import_cjs.cjs)({ src, dest, target, platform, override, debug });
  }
});
cli.start();
