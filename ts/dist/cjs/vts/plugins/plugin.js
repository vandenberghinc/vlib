...
class Source {
  ...
  constructor(path, type, data, ts_src, config, in_memory) {
    ...
    this.path = path instanceof import_vlib.Path ? path : new import_vlib.Path(path);
    this.type = type;
    this.is_src = this.type === "src";
    this.is_dist = this.type === "dist";
    this.non_unique_id = new import_vlib.Path(this.path.path);
    this.data = data;
  ...
  get safe_ts_src() {
    ...
    if (!(this.ts_src instanceof import_vlib.Path))
