...
class Tester {
  ...
  async run() {
    ...
    if (config.env?.length) {
      ...
      for (const e of env) {
        if (config.debug >= 1)
          import_vlib.logger.marker(`Importing environment file ${e}`);
        vlib.env.from(e);
      }
    }
 ...
 - ${i}`).join("")}`);
    ...
    const results_dir = new import_vlib.Path(config.results);
...
(function(Tester2) {
  Tester2.config_scheme = {
    ...
    env: {
      type: ["array", "string"],
      // Also accept single string type for config files.
      required: false,
      default: [],
      value_scheme: { type: "string" }
    },
