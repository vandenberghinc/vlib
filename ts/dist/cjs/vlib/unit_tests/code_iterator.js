"use strict";
var import_vtest = require("@vandenberghinc/vlib/vtest");
var import_path = require("../generic/path.js");
var import_iterator = require("../code/iterator.js");
var import_node_url = require("node:url");
const import_meta = {};
const tests = new import_vtest.Module({ name: "vlib/code_iterator" });
tests.add("success:some_lt:1", "success", () => {
  const txt = `
    ...
    * ];
    ...
    `.dedent();
  const it = new import_iterator.Iterator({
    data: txt,
    language: { string: ["`"] }
  });
  it.jump_to(4, {
...
tests.add("temporary_unterminated_code_line:1", "success", async () => {
  const txt = await new import_path.Path((0, import_node_url.fileURLToPath)(import_meta.url).split("/dist")[0] + "/src/vlib/code/iterator.ts").load();
  const start = 185;
  const end = 2955;
  const it = new import_iterator.Iterator({
    data: txt,
