...
export declare namespace Entry {
     ...
     */
    type Plain<T extends CastType = CastType, // type of entry
    V extends Entry.Type.Cast<T> = Entry.Type.Cast<T>, // value of entry
    P extends ObjOrArr = ObjOrArr, // parent object for callbacks.
    Derived extends Record<string, any> = {}> = ((Derived extends Record<string, any> ? Derived : {}) & {
        /**
         * The value type.
...
export declare namespace Entries {
     ...
     */
    type Opts<Derived extends Record<string, any> = {}> = NonNullable<Entry.Plain<any, any, any, Derived>["schema"]>;
    /** Extract the value‐type for a normal (named) Scheme.Opts object */
...
export declare namespace ValueEntries {
     ...
     */
    type Opts<Derived extends Record<string, any> = {}> = NonNullable<Entry.Plain<any, any, any, Derived>["value_schema"]>;
    /** Extract the value‐type for a normal (named) Scheme.Opts object */
...
export declare namespace TupleEntries {
     ...
     * Inferrable input options for constructing a new `value_schema: Entry` object.
     */
    type Opts<Derived extends Record<string, any> = {}> = NonNullable<Entry.Plain<any, any, any, Derived>["tuple"]>;
...
export type InferEntry<E extends Entry.Opts> = E extends Entry.Plain ? E extends {
 ...
 */
export type ValueEntriesParentType<E extends CastType | Entry.Plain, Default extends "array" | "object" = "array"> = E extends CastType ? Entry.Type.Cast<E> extends readonly any[] ? "array" : "object" : E extends Entry.Plain ? E extends {
    default: infer D;
...
export type InferValueEntries<ParentT extends "array" | "object", V extends ValueEntries.Opts> = V extends Entry.Plain ? ParentT extends "array" ? InferEntry<V>[] : Record<string, InferEntry<V>> : V extends CastType ? ParentT extends "array" ? Entry.Type.Cast<V>[] : Record<string, Entry.Type.Cast<V>> : never;
/** Extract the value‐type for a tuple, preserving tuple positions */
export type InferTupleEntries<T extends readonly Entry.Opts[]> = {
    [I in keyof T]: T[I] extends Entry.Opts ? InferEntry<T[I]> : never;
};
/** Is entry optional */
type IsOptional<E extends Entry.Opts> = E extends Entry.Plain ? [
    E["required"]
] extends [false] ? true : E["required"] extends Function ? true : false : false;
/** Wrap maybe as optional (old). */
type MaybeOptional<E extends Entry.Opts, T extends any> = IsOptional<E> extends true ? T | undefined : T;
/** Extract the value‐type for a normal (named) Entries.Opts object */
export type InferEntries<S extends Entries.Opts> = {
    [K in keyof S]: S[K] extends Entry.Plain ? MaybeOptional<S[K], InferEntry<S[K]>> : S[K] extends CastType ? MaybeOptional<S[K], Entry.Type.Cast<S[K]>> : unknown;
};
export {};
