...
class Debug extends import_callable.Callable {
  ...
  constructor(opts) {
    ...
    super();
    this.pipe = new import_pipe.Pipe({
      log_level: 0,
      out: console.log,
      err: console.error
    });
    this.pipe.log_level.set(level instanceof import_directives.ActiveLogLevel ? level.n : level);
   ...
   */
  call(log_level, ...args) {
    this.pipe.log(this.level, import_directives.Directive.debug, new import_source_loc.SourceLoc(2), log_level, ...args);
  }
  /**
   ...
   */
  raw(level, ...args) {
    this.pipe.raw(this.level, import_directives.Directive.debug, import_directives.Directive.raw, new import_source_loc.SourceLoc(1), level, ...args);
  }
  dump(level, ...args) {
    this.pipe.raw(this.level, import_directives.Directive.debug, import_directives.Directive.raw, new import_source_loc.SourceLoc(1), level, ...args);
  }
