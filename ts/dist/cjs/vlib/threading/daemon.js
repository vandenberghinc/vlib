...
class Daemon {
  ...
  constructor(options) {
    ...
    this.max_restart_delay_ms = Math.max(this.min_restart_delay_ms, options.max_restart_delay_ms ?? 3e4);
    this.restart_jitter_ratio = this.clamp_number(options.restart_jitter_ratio ?? 0.2, 0, 1);
    this.backoff_reset_after_ms = Math.max(0, options.backoff_reset_after_ms ?? 6e4);
    this.worker = null;
    this.running = false;
    this.stopping = false;
  ...
  get_stats() {
    const now = Date.now();
    const uptime_ms = this.start_time_ms != null && this.running && this.worker ? Math.max(0, now - this.start_time_ms) : 0;
    return {
      name: this.name,
  ...
  spawn_worker() {
    ...
    this.last_exit_code = null;
    this.last_exit_reason = null;
    const node_worker = new import_worker_threads.Worker(this.daemon_path, {
