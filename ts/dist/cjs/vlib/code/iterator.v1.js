...
class Iterator {
  ...
  constructor(state, opts) {
    if (state instanceof Iterator) {
      ...
      if (typeof state === "object" && "data" in state)
        ...
        throw new TypeError(`Invalid type for state, expected Source or State<S>, got: ${(0, import_index_m_node.value_type)(state)}`);
  ...
  consume_comment(slice) {
    ...
    if (end === -1) {
      this.stop();
      return slice ? this.source.data.slice(start) : void 0;
    }
    if (this.is_comment.type === "line") {
      this.pos = end;
      this.init();
      return slice ? this.source.data.slice(start, end) : void 0;
    }
    const ctx = {
      pos: this.pos,
      line: this.line,
      ...
      while (ctx.pos < this.end) {
        ...
        if (char === this.is_comment.close[is_comment_pos]) {
          ...
          if (is_comment_pos === this.is_comment.close.length) {
            ...
            this.pos = ctx.pos + 1;
            this.init();
            return slice ? this.source.data.slice(start, ctx.pos + 1) : void 0;
          }
        } else if (is_comment_pos > 0)
          is_comment_pos = 0;
        this.minor_advance(ctx);
      }
    }
    return slice ? this.source.data.slice(start, ctx.pos) : void 0;
  }
  /**
