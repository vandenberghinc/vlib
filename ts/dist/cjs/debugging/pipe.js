...
class Pipe {
  ...
  pipe(args, directives) {
    ...
    if (!is_raw) {
      ...
      if (!loc.is_unknown() && mode === import_directives.Directive.debug) {
        const id = loc.caller === "<unknown>" || loc.caller === "<root>" ? loc.id : `${loc.filename}:${loc.line}:${loc.column}:${loc.caller}`;
        msg.push(`[${id}] `);
      }
  ...
  loggers(active_log_level) {
    const active_log = active_log_level == null ? this.log_level : new import_directives.ActiveLogLevel(active_log_level);
    return {
      log: (...args) => {
