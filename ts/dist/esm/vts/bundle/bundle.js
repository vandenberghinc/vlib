...
import { Color, Colors, log, Path, Utils } from "../../vlib/index.js";
// ------------------------------------------------------------------------------------------------------------------------
// Utilties.
/**
 ...
 * @private
 */
export const create_esbuild_err = (warning) => {
    let output;
    if (warning.location) {
...
export async function bundle(options) {
    let { include = [], externals = [], output = undefined, platform = 'browser', format = 'iife', target = 'es2021', minify = false, sourcemap = false, // 'inline'
    error_limit = 25, extract_inputs = false, tree_shaking = undefined, debug = false, postprocess = undefined, } = options;
    // Variables.
    const errors = [];
