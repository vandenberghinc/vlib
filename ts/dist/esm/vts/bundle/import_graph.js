            ...
            setup: (build) => {
                ...
                build.onLoad({ filter: /.*/ }, async (args) => {
                    ...
                    try {
                        ...
                        if (this.options.debug?.on(1))
                            this.options.debug(`${Color.red("Error")} reading file ${file_path}: ${err instanceof Error ? err.message : String(err)}`);
                    }
                ...
                build.onEnd((result) => {
                    if (this.options.debug?.on(1))
                        this.options.debug(Color.gray(`[ImportGraph] Build ended. Graph has ${this.graph.size} nodes`));
                    // If we have a metafile, use it to enhance our graph
                    if (result.metafile) {
                        if (this.options.debug?.on(1))
                            this.options.debug(Color.gray("[ImportGraph] Enhancing graph with metafile data"));
                        this.enhance_with_metafile(result.metafile);
                    }
                    // Debug: Show sample of what we captured
                    if (this.graph.size > 0) {
                        if (this.options.debug?.on(1))
                            this.options.debug(Color.gray("[ImportGraph] Sample nodes with imports:"));
                        let count = 0;
                        for (const [file, node] of this.graph.entries()) {
                            if (node.imports.size > 0 && count < 3) {
                                if (this.options.debug?.on(1))
                                    this.options.debug(Color.gray(`  ${file} imports ${node.imports.size} files`));
                                count++;
                            }
                        }
                    }
