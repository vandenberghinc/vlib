...
export class Transformer {
    ...
    static id = new Plugin.Id("vts");
    /** The log id for the alias log methods. */
    static log_id = new Path("/vts", false);
    ...
    init_source(file_path, in_memory = false) {
        ...
        if (type === "dist") {
            ...
            if ((in_memory && this.config.files?.has(ts_path))
                || (!in_memory && Path.exists(ts_path))) {
                ts_src = new Path(ts_path, false);
            }
            else if (this.tsconfig
        ...
        // Create source.
        const src = new Source(file_path, type, in_memory ? this.get_in_memory_file(file_path) : undefined, ts_src, this.config, in_memory);
            ...
            for (const plugin of plugins) {
                ...
                if ((source.is_src && plugin.has_src) ||
                    ...
                    if (p instanceof Promise)
                        await p;
                    if (source.changed && old !== undefined)
    ...
    static async run_multiple(transformers, opts) {
        const ts = transformers.map(t => t instanceof Transformer
            ? t
