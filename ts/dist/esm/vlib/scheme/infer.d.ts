...
type InferScheme<S extends Infer.Scheme.S> = {
...
type InferTupleScheme<T extends readonly (EntryObject | Cast.Castable)[]> = {
...
export declare namespace Infer {
    /** Extract the created value type of an entry, recursing into nested schemes & tuples */
    type Entry<E extends EntryObject> = InferEntry<E>;
    /** Extract the value‐type for a normal (named) Scheme.Opts object */
    type Scheme<S extends Scheme.S> = InferScheme<S>;
    namespace Scheme {
        /** Base for generic, could be useful for user. */
        type S = Record<string, EntryObject | Cast.Castable>;
    }
    /** Extract the value‐type for a tuple, preserving tuple positions */
    type Typle<T extends readonly (EntryObject | Cast.Castable)[]> = InferTupleScheme<T>;
}
