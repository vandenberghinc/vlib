...
export class Iterator {
    ...
    consume_comment(slice) {
        ...
        if (end === -1) {
            this.stop();
            return (slice ? this.source.data.slice(start) : undefined);
        }
        // Jump to the end of the line if this is a line comment.
        if (this.is_comment.type === "line") {
            this.pos = end;
            this.init();
            return (slice ? this.source.data.slice(start, end) : undefined);
        }
            ...
            while (ctx.pos < this.end) {
                ...
                if (char === this.is_comment.close[is_comment_pos]) {
                    ...
                    if (is_comment_pos === this.is_comment.close.length) {
                        ...
                        this.init();
                        return (slice ? this.source.data.slice(start, ctx.pos + 1) : undefined);
                    }
                }
        ...
        return (slice ? this.source.data.slice(start, ctx.pos) : undefined);
    }
    /**
