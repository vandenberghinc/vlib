...
*/
export type PrimitiveStringLiteral<T extends string> = T extends `${infer _}` ? (string extends T ? never : T) : never;
/**
...
*/
export type SnakeToPascalCase<S extends string> = S extends `${infer Head}_${infer Tail}` ? `${Capitalize<Head>}${SnakeToPascalCase<Tail>}` : Capitalize<S>;
