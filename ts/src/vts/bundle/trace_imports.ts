...
/**
 * Package.json minimal typing for fields we consult.
 */
...
export class ImportTracer {
     ...
     */
    private supported_exts: ReadonlyArray<string> = [
        ".ts",
        ".tsx",
        ".mts",
        ".cts",
     ...
     */
    private parsed_imports_cache: Map<string, ReadonlyArray<string>> = new Map();

    /**
     * Cache: resolved specifier -> absolute path, keyed by `${importer}::${specifier}`.
     */
    private resolution_cache: Map<string, string | null> = new Map();

    /**
     * Cache: existence checks for absolute paths.
     */
    private file_exists_cache: Map<string, boolean> = new Map();

    /**
     * Cache: realpath resolution per absolute path (handles symlinks in node_modules like pnpm/yarn).
     */
    private realpath_cache: Map<string, string> = new Map();
     ...
     */
    trace(traces: TraceRequest[], import_limit = 100): ImportChain[] {
        ...
        for (const req of traces) {
            ...
            const to_abs =
                this.resolve_import_specifier(from_abs, req.to) ??
     ...
     */
    format_import_chains(trace_results: Omit<ImportChain, "from">[], indent = "", limit = -1): string[] {
     ...
     */
    private find_all_paths(start_abs: string, goal_abs: string, max_depth: number): string[][] {
        const chains: string[][] = [];
        const stack: string[] = [];
        ...
        const expanded_in_trace = new Set<string>();

        // console.log(`Tracing imports from "${start_abs}" to "${goal_abs}"`);
        const dfs = (current_abs: string, depth: number, on_path: Set<string>): void => {
        ...
        dfs(start_abs, 0, new Set<string>());
        return chains;
    }
     ...
     */
    private get_parsed_imports(file_abs: string): ReadonlyArray<string> {
     ...
     */
    private resolve_import_specifier(importer_abs: string, specifier: string): string | null {
     ...
     */
    private resolve_file_like(base_path: string): string | null {
        const as_file = this.try_file(base_path);
        if (as_file) return as_file;
     ...
     */
    private resolve_node_package(importer_abs: string, specifier: string): string | null {
     ...
     * @returns Absolute resolved file path or `null`.
     */
    private resolve_package_entry(pkg_root: string, subpath: string): string | null {
        ...
        const resolve_rel = (rel: string): string | null => this.resolve_file_like(path.join(pkg_root, rel));
        ...
        const fields: ReadonlyArray<keyof PackageJson> = ["module", "browser", "main"];
     ...
     */
    private normalize_to_abs(p: string): string {
     ...
     * @param b Second path.
     */
    private paths_equal(a: string, b: string): boolean {
     ...
     */
    private is_relative_or_absolute(s: string): boolean {
     ...
     */
    private is_absolute(p: string): boolean {
     ...
     */
    private strip_comments(src: string): string {
     ...
     */
    private safe_read_file(abs: string): string | null {
        try {
     ...
     */
    private try_file(p: string): string | null {
     ...
     * @param p Path to test.
     */
    private try_dir(p: string): string | null {
        const n = path.normalize(p);
        const cached = this.file_exists_cache.get(n);
        if (cached !== undefined) {
     ...
     */
    private match_all(text: string, rx: RegExp, out: Set<string>): void {
     ...
     */
    private split_package_and_subpath(specifier: string): { pkg_name: string; subpath: string } {
     ...
     */
    private read_package_json(pkg_json_abs: string): PackageJson | null {
        try {
            const raw = fs.readFileSync(pkg_json_abs, { encoding: "utf8" });
            const parsed = JSON.parse(raw) as unknown;
            if (this.is_package_json(parsed)) return parsed;
            return null;
        } catch {
            return null;
        }
     ...
     */
    private is_package_json(v: unknown): v is PackageJson {
     ...
     */
    private resolve_exports_target(exp: unknown, key: string): string | Record<string, unknown> | null {
        ...
        if (typeof exp === "object" && exp !== null) {
            ...
            if (typeof value === "object" && value !== null) {
                ...
                // Preference order
                const pref: ReadonlyArray<string> = ["import", "browser", "default", "module", "node"];
                for (const k of pref) {
                    const got = m[k];
     ...
     */
    private first_string_in_export_map(obj: Record<string, unknown>): string | null {
        const pref: ReadonlyArray<string> = ["import", "browser", "default", "module", "node"];
        for (const k of pref) {
            const v = obj[k];
     ...
     */
    private strip_dot_slash(rel: string): string {
     ...
     */
    private to_canonical(abs: string): string {
        const n = path.normalize(abs);
        const cached = this.realpath_cache.get(n);
        if (cached) return cached;
        try {
            // Prefer native if available for performance
            const rp = (fs.realpathSync as unknown as { native?: (p: string) => string }).native?.(n) ?? fs.realpathSync(n);
