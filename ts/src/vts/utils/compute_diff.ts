...
export function compute_diff({ new: new_data, old, prefix = "", trim = true, trim_keep = 3 } : {
    ...
    const diffs: Change[] = diffLines(old, new_data).reduce<Change[]>((acc, part) => {
    ...
    const diff_lines: string[][] = diffs.map((part) => part.value.split(/\r?\n/));
    ...
    for (let part_index = 0; part_index < diffs.length; ++part_index) {
        ...
        for (let line_index = 0; line_index < lines.length; ++line_index) {
            ...
            if (line_index === lines.length - 1 && line === "") continue;
            ...
            if (
                ...
                if (
                    ...
                    dumped_lines.push(
                        `${whitespace_prefix} ${"".padEnd(max_line_nr_length, " ")} | ${line_prefix} ${Color.italic(
                            "... unchanged ..."
                        )}`
                    );
                    last_dots = true;
            ...
            dumped_lines.push(
                `${whitespace_prefix} ${String(display_nr).padEnd(max_line_nr_length, " ")} | ${line_prefix} ${line}`
            );
            last_dots = false;
        }
    }
