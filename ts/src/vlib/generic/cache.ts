...
export class Cache<K extends string | number | symbol = string, V = any> {
    ...
    private cleanup_interval_id: NodeJS.Timeout | undefined;
     ...
     */
    private _start_cleanup_interval(cleanup_interval: number): void {
        this.cleanup_interval_id = setInterval(() => {
            // Handle TTL eviction
            if (this.ttl !== undefined) {
                const now = Date.now();
                const keys_to_delete: K[] = [];
     ...
     */
    private _estimate_size(value: any, visited: WeakSet<any>): number {

        // Circular reference guard.
        if (visited.has(value)) return 0; // Circular reference
        if (typeof value === 'object' && value !== null) visited.add(value);
     ...
     */
    private _get_total_memory_size(): number {
        let total_size = 0;
