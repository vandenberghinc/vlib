...
export class Debug extends Callable<Args, void> {
    ...
    call(log_level: number | Args[0], ...args: Args): void {
        this.pipe.log(this.level, Directive.debug, new SourceLoc(2), log_level, ...args);
    }
    ...
    raw(level: number | any, ...args: Args): void {
        this.pipe.raw(this.level, Directive.debug, Directive.raw, new SourceLoc(1), level, ...args); // do not use console.log here since we join args without a space unlike console.log.
    }
