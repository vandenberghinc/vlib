...
export class Git {
    ...
    // Git preperation.
    async _git_prepare(remote: string, dest: string, branch: string) {
    ...
    async remove_commit_history({branch}: {branch: string}) {
        return new Promise<string | undefined>(async (resolve) => {
    ...
    is_ignored(path: Path | string, _is_relative = false) {
    ...
    async list_large_files({
        ...
        const list: {path: Path, size: number}[] = [];
        let listed = 0;
