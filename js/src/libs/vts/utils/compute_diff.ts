...
import { Color, debugging, debug } from '../../../index.js';
 ...
 * 
 */
export function compute_diff({ new: new_data, old, log_level, prefix = "", trim = true, trim_keep = 3, detected_changes } : {
    new: string,
    old: string,
...
}): { status: "identical", changes?: never } | { status: "diff", changes: Change[] } {
    ...
    if (old === new_data) {
        if (debug.on(0, level)) debug.raw(level, prefix, "Old and new data are identical.");
        return { status: "identical" };
    }

    // Otherwise compute a line-by-line diff.
    if (debug.on(0, level)) debug.raw(level, prefix, detected_changes ?? "Detected changes between old and new data, computing diff...");
    // console.log("---------\n" + old + "\n---------\n" + new_data + "\n---------");
    ...
    diffs.forEach((part, index) => {
        ...
        diff_lines[index].walk((line, i, arr) => {
            ...
            if (trim && !part.added && !part.removed && !(
                i < trim_keep || i >= arr.length - trim_keep
            )) {
                if (!last_dots && (i === trim_keep || i === arr.length - trim_keep - 1)) {
                    if (debug.on(0, level)) debug.raw(
                        level,
                        `${whitespace_prefix} ${String().padEnd(max_line_nr_length, ' ') } | ${line_prefix} ${Color.italic("... unchanged ...")}`
                    );
                    last_dots = true;
                }
                return ;
            ...
            if (debug.on(0, level)) debug.raw(
                level,
                `${whitespace_prefix} ${String(local_line_nr != null ? local_line_nr : line_nr).padEnd(max_line_nr_length, ' ') } | ${line_prefix} ${line}`
            );
            last_dots = false;
        });
        --line_nr;
