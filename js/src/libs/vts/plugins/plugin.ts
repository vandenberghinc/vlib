...
import { EnforceOne, Merge } from "../../../types/index.m.js";
    ...
    add_change(plugin: Plugin | Plugin.Id, old: { data: string }): void {
        ...
        if (this.debug.on(Source.log_level_for_changes)) {
            compute_diff({
                ...
                old: old.data,
                prefix: "    ",
                detected_changes: `Detected changes made by plugin "${plugin.id}", computing diff...`,
            });
        }
    }
                ...
                try {
                    ...
                    for (let i = 0; i < this.changes.length; i++) {
                        ...
                        compute_diff({
                            ...
                            trim_keep: 2,
                            log_level: 0,
                            detected_changes: `Detected changes in source data, computing diff...`,
                        });
                    }
