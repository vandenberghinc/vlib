...
cli.command({
    ...
    callback: async ({
        ...
        for (const source of all_sources) {
            ...
            // Pull through git.
            if (git != null) {
                if (!Array.isArray(git)) { cli.throw(`The git parameter must be an array of git remotes.`); }
                for (const g of git) {
            ...
            // Pull through ssh.
                ...
                if (!Array.isArray(ssh)) { cli.throw(`The ssh parameter must be an array of ssh remotes.`); }
                for (const s of ssh) {
