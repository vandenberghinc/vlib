...
export class Plugin {
    ...
    async run() {
        ...
        const matched_files = include_patterns.flatMap(pattern => glob_sync(pattern, { cwd: tsconfig_dir, absolute: true, ignore: exclude_patterns }));
        ...
        for (const file_path of matched_files) {
            ...
            if (this.config.dirname && dist_path) {
                promises.push(dirname_plugin(source_path, dist_path, tsconfig_path.str(), tsconfig_dir));
            }
            // If author sync is enabled, process source file
            if (this.config.header?.author) {
                promises.push(header_plugin(source_path, this.config.header.author, this.config.header.start_year));
            }
            // If author sync is enabled, process source file
            if (this.config.no_debug && dist_path) {
                promises.push(no_debug_plugin(dist_path));
            }
            // Version plugin
            if (pkg_version && dist_path) {
                fill_version(dist_path, undefined, false, pkg_version);
            }
            // Fill templates.
