...
const cli = new vlib.CLI({
    name: "vts", version: "1.0.0", main: {
        ...
        callback: async ({ tsconfig: t = "./", header = false, author = undefined, copyright = undefined, dirname = false, version, no_debug = false, templates, }) => {
    ...
    commands: [
        // Fill templates.
        {
            ...
            callback: async ({ src, templates, allow_not_found = false, }) => await fill_templates(src, templates, allow_not_found),
        },
        // Fill version.
        {
            id: "--fill-version",
            ...
            callback: async ({ src, pkg = "./package.json", allow_not_found = false, }) => await fill_version(src, pkg, allow_not_found),
        },
        ...
        // Convert ESM to CJS.
        {
            ...
            callback: async ({ src, dest, target, platform, override = false, quiet = false, }) => await cjs({ src, dest, target, platform, override, quiet }),
        },
    ]
});
cli.start();
