...
// CLI.
const cli = new vlib.cli.CLI({ name: "vts", version: "1.0.0", strict: true });
cli.main({
    ...
    callback: async ({ tsconfig = process.cwd(), include = [], exclude = [], header, dirname = false, version, no_debug = false, templates, exact_templates, yes = false, async: parallel = false, debug = 0, }) => {
        if (header && !header.author) {
            cli.throw("The --header --author argument is required when using the header plugin.");
        }
...
cli.command({
    ...
    args: [
        ...
        { id: ["--platform", "-p"], enum: ["browser", "node", undefined], required: false, description: "The ES platform, 'browser' or 'node'." },
    ...
    callback: async ({ src, dest, target, platform, override = false, debug = 0, }) => await cjs({ src, dest, target, platform, override, debug }),
});
cli.start();
