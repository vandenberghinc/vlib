...
const cli = new vlib.CLI({
    name: "vrepo", version: "1.0.2", commands: [
        // ---------------------------------------------------------------
        // Remote commands.
        // Push.
        {
        ...
        // Pull.
        {
            ...
            callback: async ({ source = null, sources = null, git = null, ssh = null, forced = false, del = false, _command = null, }) => {
                // Check args.
                ...
                for (const source of all_sources) {
                    ...
                    // Pull through git.
                    if (git != null) {
                        const remote = repo.config.git.remotes.find(item => item.remote === git);
                        if (!remote) {
                            return cli.throw_error(`Git remote "${git}" does not exist.`);
                        }
        ...
        // Add remote.
        {
            ...
            async callback({ source = null, ssh = null, git = null, remote = "origin", destination = null, branch = "main", _command = null, }) {
                // Create sources array.
                if (typeof source !== "string") {
                    source = "./";
                }
        ...
        // Publish.
        {
            id: new vlib.CLI.Query.And("--npm", "--publish"),
        ...
        {
            id: new vlib.CLI.Query.And("--npm", "--link"),
            ...
            callback: async ({ source = null, sources = null, dependencies = [], }) => {
                if (!dependencies?.length) {
                    cli.throw_error(`The argument --dependencies is required.`);
                }
                // Create sources array.
                const all_sources = [];
                if (typeof source === "string") {
                ...
                for (const source of all_sources) {
                    ...
                    if (!vrepo_src.exists()) {
                        cli.throw_error(`The vrepo source path "${source}" does not exist. Create an empty json file to continue.`);
                        // vrepo_src.save_sync("{}");
                    }
                    const repo = new Repo({
                        source,
                    ...
                    if (!Object.keys(repo.config.npm.links).length && !dependencies.length) {
                        cli.throw_error(`Argument --dependencies is required when this command is executed for the first time.`);
                    }
                    ...
                    for (const dependency of dependencies) {
                        ...
                        if (!dependency_path.exists()) {
                            cli.throw_error(`The dependency path "${dependency}" does not exist.`);
                        }
                        const pkg = await dependency_path.load({ type: "object" });
                        if (pkg.name == null || pkg.version == null) {
                            cli.throw_error(`The dependency package.json file does not contain a name or version field.`);
                        }
                        cmd += " " + pkg.name;
        ...
        {
            id: new vlib.CLI.Query.And("--npm", "--unlink"),
            ...
            callback: async ({ source = null, sources = null, install = false, }) => {
                ...
                const unlink = async (source) => {
                    ...
                    if (!package_json_path.exists()) {
                        cli.throw_error(`The package.json file does not exist in the source path "${source}".`);
                    }
                    // Load package.json.
                    const package_json = await package_json_path.load({ type: "object" });
                    if (package_json.name == null || package_json.version == null) {
                        cli.throw_error(`The package.json file does not contain a name or version field.`);
                    }
        ...
        {
            id: new vlib.CLI.Query.And("--git", "--remove-history"),
            ...
            callback: async ({ source = null, sources = null, }) => {
                ...
                for (const source of all_sources) {
                    ...
                    // if (!found) {
                    //     cli.throw_error(`No git remote "${git}" does not exist.`);
                    // }
        ...
        // Remove git cache.
        {
            id: new vlib.CLI.Query.And("--git", "--remove-cache"),
         ...
         */
        {
            ...
            callback: async ({ target: target_dir, exclude = [], }) => {
                if (!new vlib.Path(target_dir).exists()) {
                    cli.throw_error(`The target directory "${target_dir}" does not exist.`);
                }
