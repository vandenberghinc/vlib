...
async function cjs({ src, dest, target, platform, override = false, quiet = false }) {
  const src_path = new import__.Path(src);
  if (!src_path.exists()) {
  ...
  const dest_path = new import__.Path(dest);
  if (dest_path.exists()) {
    if (override) {
  ...
  for (const esm of esm_files) {
    ...
    if (ext !== ".js") {
      if (!quiet) {
        import__.logger.marker(`Copying asset ${esm}`);
      }
    ...
    try {
      ...
      if (!quiet) {
        import__.logger.marker(`Transformed ${esm} [${import__.Utils.format_bytes(Buffer.byteLength(result.code, "utf8"))}]`);
      }
    } catch (err) {
