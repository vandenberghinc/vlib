...
class Transformer {
  ...
  static log_id = new import__.Path("/vts", false);
  ...
  init_source(file_path, in_memory = false) {
    const f = new import__.Path(file_path, false);
    ...
    if (type === "dist") {
      ...
      if (in_memory && this.config.files?.has(ts_path) || !in_memory && import__.Path.exists(ts_path)) {
        ts_src = new import__.Path(ts_path, false);
      } else if (this.tsconfig && (root_dir = this.tsconfig.compilerOptions?.rootDir ? import_path.default.resolve(this.tsconfig_base?.path || "", this.tsconfig.compilerOptions.rootDir) : this.tsconfig_base?.path || void 0) && (relative_path = import_path.default.relative(root_dir, ts_path)) && (full_ts_path = import_path.default.resolve(root_dir, relative_path)) && (in_memory && this.config.files?.has(full_ts_path) || !in_memory && import__.Path.exists(full_ts_path))) {
        ts_src = new import__.Path(full_ts_path, false);
      }
  ...
  async init_sources() {
    ...
    const cbp = import__.Path.find_common_base_path(Array.from(this.sources.keys()));
    if (cbp) {
  ...
  async create_include_patterns() {
    ...
    const process_include_pattern = (include, in_memory = false) => {
      ...
      const path = new import__.Path(include);
      if (!path.exists()) {
...
${include_patterns.map((l, i) => `      - ${l.replace(/^[./]*/, "")}`).join("\n")}`);
    const matched_files = include_patterns.length === 0 ? [] : await (0, import_fast_glob.default)(include_patterns.map((p) => {
      const path = new import__.Path(p);
  ...
  static resolve_tsconfig(path) {
    let tsconfig = new import__.Path(path);
    if (!tsconfig.exists()) {
