...
async function bundle(options) {
  ...
  if (typeof output === "string") {
    await new import__.Path(output).save(bundled_code ?? "");
  } else if (Array.isArray(output)) {
    for (let i = 0; i < output.length; i++) {
      await new import__.Path(output[i]).save(bundled_code ?? "");
    }
  }
  if (log_level >= 1) {
    const first_path = typeof output === "string" ? output : Array.isArray(output) ? output[0] : void 0;
    if (first_path != null) {
      const p = new import__.Path(first_path);
      import__.logger.marker(`Bundled ${p.full_name()} (${p.str()}) [${import__.Utils.format_bytes(p.size)}].`);
    }
  }
  return {
    code: bundled_code,
    ...
    debug() {
      ...
      if (typeof bundled_code === "string" && bundled_code !== "") {
        lines.push(`Generated code of ${import__.Utils.format_bytes(Buffer.byteLength(bundled_code, "utf8"))}`);
      }
      return lines.join("\n");
    }
  };
...
const format_esbuild_warning_error = (warning) => {
  ...
  if (warning.location) {
    ...
    output = `${import__.Color.cyan(warning.location.file)}:${import__.Color.yellow(warning.location.line)}:${import__.Color.yellow(warning.location.column)} - ${import__.Color.yellow("warning")} [esbuild${warning.id === "" ? "" : `-${warning.id}`}]: ${warning.text}

` + import__.Colors.bright_bg.white + import__.Colors.black + warning.location.line + import__.Colors.end + "    " + trimmed_line + "\n" + import__.Colors.bright_bg.white + import__.Colors.black + " ".repeat(warning.location.line.toString().length) + import__.Colors.end + " ".repeat(4 + warning.location.column - removed_start_indent) + import__.Color.red("~".repeat(warning.location.length));
  } else {
    output = `${import__.Color.yellow("warning")} [esbuild${warning.id === "" ? "" : `-${warning.id}`}]: ${warning.text}`;
  }
  if (Array.isArray(warning.notes)) {
    for (const note of warning.notes) {
      if (note.location) {
        ...
        output += `
    ${import__.Color.cyan(note.location.file)}:${import__.Color.yellow(note.location.line)}:${import__.Color.yellow(note.location.column)} - ${import__.Color.gray("note")}: ${note.text}

` + import__.Colors.bright_bg.white + import__.Colors.black + note.location.line + import__.Colors.end + "        " + trimmed_line + "\n" + import__.Colors.bright_bg.white + import__.Colors.black + " ".repeat(note.location.line.toString().length) + import__.Colors.end + " ".repeat(8 + note.location.column - removed_start_indent) + import__.Color.red("~".repeat(note.location.length));
      } else {
        output += `
    ${import__.Color.gray("note")}: ${note.text}`;
      }
      if (note.suggestion) {
...
class ImportTracker {
  ...
  show_import_chain(include, error, indent = "") {
    ...
    const prepare_paths = (paths) => {
      ...
      if (p.length > 0) {
        const common_base = import__.Path.find_common_base_path(p);
        if (common_base) {
          const slice_start = new import__.Path(common_base).abs().str().length;
    ...
    const walk = (curr, chain) => {
      if (curr === error_path || pathlib.resolve(curr) === error_path) {
        const lines = prepare_paths(chain.concat(curr)).map((l, i) => false ? import__.Color.italic(l) : `${indent}  ${import__.Color.italic(l)}`).map((l, i, arr) => i === arr.length - 1 ? l : `${l} ${import__.Color.blue("\u2192")}`).map((l, i, arr) => {
          if (!l) {
            return l;
          }
        ...
        out_lines.push(`${indent}${import__.Color.gray("note")}: Import chain for error 
${lines}`);
        return;
      }
