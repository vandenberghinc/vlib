...
async function perform({ results, module: module2 = import__.cli.get({ id: "--module", required: false }), target = import__.cli.get({ id: "--target" }), stop_on_failure = import__.cli.present("--stop-on-failure"), stop_after = import__.cli.get({ id: "--stop-after", required: false }), debug = import__.cli.get({ id: "--debug", def: 0, type: "number" }), interactive = import__.cli.present("--interactive"), all_yes = import__.cli.present(["--yes", "-y"]), repeat = import__.cli.get({ id: "--repeat", def: 0, type: "number" }), list_modules = import__.cli.present(["--list-modules", "--list"]), no_changes = import__.cli.present(["--no-changes", "-nc"]), refresh = import__.cli.present("--refresh") ? true : import__.cli.get({ id: "--refresh", def: false, type: ["string", "boolean"] }) }) {
  if (list_modules) {
