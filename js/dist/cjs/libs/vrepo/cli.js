    ...
    // Link - Simply call link on target vrepo projects.
    {
      ...
      callback: async ({ source = null, sources = null, dependencies = [] }) => {
        ...
        for (const source2 of all_sources) {
          ...
          if (!vrepo_src.exists()) {
            cli.throw_error(`The vrepo source path "${source2}" does not exist. Create an empty json file to continue.`);
          }
          const repo = new import_repo.Repo({
            source: source2,
            git: false,
      ...
      callback: async ({ source = null, sources = null, install = false }) => {
        ...
        const unlink = async (source2) => {
          ...
          if (!package_json_path.exists()) {
            cli.throw_error(`The package.json file does not exist in the source path "${source2}".`);
          }
